...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        updates = [(param_vars[i], param_vars[i] + (vel_vars[i]*self.momentum) - (self.learning_rate*grad_tensors[i])) for i in range(len(param_vars))]
        for i in range(len(vel_vars)):
            updates.append((vel_vars[i], (vel_vars[i]*self.momentum) - (self.learning_rate*grad_tensors[i])))
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            for i in range(len(train_data)):
                feed_dict_train[placeholders[i]] = train_data[i]
            ...
            for i in range(len(val_data)):
                feed_dict_val[placeholders[i]] = val_data[i]
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in xrange(self.iterations):
            ...
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: tG1, placeholders[1]: tG2})
            session.run(update_ops, feed_dict={placeholders[0]: tG1, placeholders[1]: tG2})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: vG1, placeholders[1]: vG2})
