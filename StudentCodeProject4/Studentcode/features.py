...
colorbar()
    ...
    for row in range(len(datum)):
        ...
        maxValue = 0
        totalCount = 0
        for col in range(len(datum[row])):

            if(datum[row][col] > 0.81):
                totalCount += 1

                minValue = min(datum[row][col], minValue)
                maxValue = max(datum[row][col], maxValue)

        features = np.append(features, [totalCount / len(datum)])
        features = np.append(features, [maxValue / minValue])
