...
def depthFirstSearch(problem):
    ...
    while len(stack.list) != 0:
        currentCoordinates, parentCoordinates, actionTaken = stack.pop()
        statesExpanded[currentCoordinates] = {'parent': parentCoordinates, 'direction': actionTaken}
        if(problem.isGoalState(currentCoordinates)):
            goalState = currentCoordinates
            break
        nextMoves = problem.getSuccessors(currentCoordinates)
        for move in nextMoves:
            coordinates, action, cost = move
            if coordinates not in statesExpanded:
                stack.push((coordinates, currentCoordinates, action))
    steps = []
    while(statesExpanded[goalState]['parent'] != None):
        steps.insert(0, statesExpanded[goalState]['direction'])
        goalState = statesExpanded[goalState]['parent']
    return steps
...
def breadthFirstSearch(problem):
    ...
    queue = util.Queue()
    startState = problem.getStartState()
    queue.push((startState, [], None))
    statesExpanded = [startState]
    goalState = None
    while len(queue.list) != 0:
        currentCoordinates, currentAction, actionTaken = queue.pop()
        if(problem.isGoalState(currentCoordinates)):
            goalState = currentAction
            break
        nextMoves = problem.getSuccessors(currentCoordinates)
        for move in nextMoves:
            coordinates, action, cost = move
            if coordinates not in statesExpanded:
                ...
                queue.push((coordinates, currentAction + [action], action))
    return goalState
    # util.raiseNotDefined()
